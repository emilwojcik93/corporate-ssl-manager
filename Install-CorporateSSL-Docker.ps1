...
function Test-ConfigurationSummary {
    ...
    try {
        ...
        if ($envSetup -eq "configured") {
            Write-ColorLog "Environment setup: [OK] Configured in bashrc" -Level "SUCCESS" -Color Green
        } else {
        ...
        Write-ColorLog "- docker-corp status: $(if ($dockerCorpExists -eq 'exists') { '[OK] Working' } else { '[FAIL] Missing' })" -Level "INFO"
        Write-ColorLog "- Environment: $(if ($envSetup -eq 'configured') { '[OK] Configured' } else { '[FAIL] Missing' })" -Level "INFO"
