...
function Get-CorporateCertificates {
    ...
    foreach ($store in $stores) {
                    if ($VerbosePreference -eq 'Continue') { Write-ColorLog "Searching certificates in store: $store" -Level "VERBOSE" }
        ...
        try {
            ...
            foreach ($cert in $certs) {
                ...
                if ($matchesPattern -and -not $shouldExclude) {
                    if ($VerbosePreference -eq 'Continue') { Write-ColorLog "Found certificate: $subject" -Level "VERBOSE" }
