# Corporate SSL Certificate Manager for Docker - Fixed Version
# Properly configures Docker daemon and containers to use Thomson Reuters corporate certificates globally
# Fixes line ending issues and provides clean setup
...
$logDir = ".\logs"
$logFile = "$logDir\Docker_Fixed_SSL_Installation_$timestamp.log"
    ...
    foreach ($domain in $Domains) {
        try {
            $dockerCmd = if ($TestType -eq "Global") {
                "timeout 15 /usr/local/bin/docker-corp run --rm curlimages/curl:latest curl -s -o /dev/null -w '%{http_code}' '$domain' 2>/dev/null || echo '000'"
            } else {
...
function Install-FixedDockerSSLConfiguration {
    ...
    Write-ColorLog "Installing fixed Docker SSL configuration..." -Level "PROGRESS" -Color Yellow
    ...
    try {
        ...
        # Step 4: Create Docker wrapper script with proper line endings
        Write-ColorLog "Creating Docker wrapper script..." -Level "PROGRESS" -Color Yellow
...
"@
        ...
        # Write to temporary file with Unix line endings
        $tempWrapperFile = "$env:TEMP\docker-corp-wrapper"
        [System.IO.File]::WriteAllText($tempWrapperFile, $wrapperContent, [System.Text.Encoding]::UTF8)
        
        # Copy to WSL and set permissions
        $tempWrapperWSL = wsl wslpath "'$tempWrapperFile'"
        wsl bash -c "sudo cp '$tempWrapperWSL' /usr/local/bin/docker-corp" 2>$null
        wsl bash -c "sudo chmod +x /usr/local/bin/docker-corp" 2>$null
        
        # Clean up temporary file
        Remove-Item -Path $tempWrapperFile -Force -ErrorAction SilentlyContinue
...
"@
        ...
        # Write environment file with Unix line endings
        $tempEnvFile = "$env:TEMP\docker-env"
        [System.IO.File]::WriteAllText($tempEnvFile, $envContent, [System.Text.Encoding]::UTF8)
        
        $tempEnvWSL = wsl wslpath "'$tempEnvFile'"
        wsl bash -c "cp '$tempEnvWSL' ~/.docker/docker-corporate-env.sh" 2>$null
        wsl bash -c "chmod +x ~/.docker/docker-corporate-env.sh" 2>$null
...
# Main execution
Write-ColorLog "Corporate SSL Certificate Manager for Docker - Fixed Version" -Level "TITLE" -Color Magenta
...
$installResult = Install-FixedDockerSSLConfiguration -ExportedCertificates $exportedCertificates
...
Write-ColorLog "Fixed Docker: $finalSuccess/$($TestDomains.Count) domains successful" -Level "INFO"
...
$analysisResults = @{
    ...
    Configuration = "Fixed Docker SSL Setup"
}

$jsonPath = "$certificateDir\Docker_Fixed_Analysis_$timestamp.json"
...
Write-ColorLog "`nFixed Docker SSL Configuration Complete!" -Level "TITLE" -Color Magenta
...
if ($finalSuccess -eq $TestDomains.Count) {
    ...
    Write-ColorLog "Use docker-corp for HTTPS connections in containers" -Level "INFO"
}

Write-ColorLog "Docker fixed SSL certificate installation completed!" -Level "SUCCESS" -Color Green
