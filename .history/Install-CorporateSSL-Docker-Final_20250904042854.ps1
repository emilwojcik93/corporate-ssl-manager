...
function Test-ConfigurationSummary {
    Write-ColorLog "Performing comprehensive configuration verification..." -Level "PROGRESS" -Color Yellow
    
    try {
        # Test registry certificates
        $registryCount = wsl bash -c "sudo ls /etc/docker/certs.d/ 2>/dev/null | wc -l"
        Write-ColorLog "Registry certificates configured: $registryCount" -Level "INFO"
        
        # Test CA bundle
        $bundleSize = wsl bash -c "ls -lh ~/.docker/ca-bundle.crt 2>/dev/null | cut -d' ' -f5"
        if ($bundleSize) {
            Write-ColorLog "Docker CA bundle size: $bundleSize" -Level "INFO"
        } else {
            Write-ColorLog "Docker CA bundle not found" -Level "WARNING" -Color Yellow
        }
        
        # Test docker-corp script
        $dockerCorpExists = wsl bash -c "test -f /usr/local/bin/docker-corp && echo 'exists' || echo 'missing'"
        if ($dockerCorpExists -eq "exists") {
            Write-ColorLog "docker-corp script: [OK] Installed" -Level "SUCCESS" -Color Green
            
            # Test docker-corp functionality
            Write-ColorLog "Testing docker-corp functionality..." -Level "PROGRESS" -Color Yellow
            
            $googleTest = wsl bash -c "timeout 15 /usr/local/bin/docker-corp run --rm curlimages/curl:latest curl -s -o /dev/null -w '%{http_code}' https://google.com 2>/dev/null || echo '000'"
            $githubTest = wsl bash -c "timeout 15 /usr/local/bin/docker-corp run --rm curlimages/curl:latest curl -s -o /dev/null -w '%{http_code}' https://github.com 2>/dev/null || echo '000'"
            
            $googleStatus = if ($googleTest -match '^\d{3}$') { [int]$googleTest } else { 0 }
            $githubStatus = if ($githubTest -match '^\d{3}$') { [int]$githubTest } else { 0 }
            
            $googleSuccess = $googleStatus -ge 200 -and $googleStatus -lt 400
            $githubSuccess = $githubStatus -ge 200 -and $githubStatus -lt 400
            
            if ($googleSuccess) {
                Write-ColorLog "docker-corp google.com: [OK] HTTP $googleStatus" -Level "SUCCESS" -Color Green
            } else {
                Write-ColorLog "docker-corp google.com: [FAIL] HTTP $googleStatus" -Level "ERROR" -Color Red
            }
            
            if ($githubSuccess) {
                Write-ColorLog "docker-corp github.com: [OK] HTTP $githubStatus" -Level "SUCCESS" -Color Green
            } else {
                Write-ColorLog "docker-corp github.com: [FAIL] HTTP $githubStatus" -Level "ERROR" -Color Red
            }
            
        } else {
            Write-ColorLog "docker-corp script: [FAIL] Missing" -Level "ERROR" -Color Red
        }
        
        # Test environment setup
        $envSetup = wsl bash -c "grep -q 'docker-corporate-env.sh' ~/.bashrc && echo 'configured' || echo 'missing'"
        if ($envSetup -eq "configured") {
            Write-ColorLog "Environment setup: ✅ Configured in bashrc" -Level "SUCCESS" -Color Green
        } else {
            Write-ColorLog "Environment setup: ❌ Not configured in bashrc" -Level "WARNING" -Color Yellow
        }
        ...
        Write-ColorLog "• CA bundle size: $bundleSize" -Level "INFO"
        Write-ColorLog "• docker-corp status: $(if ($dockerCorpExists -eq 'exists') { '✅ Working' } else { '❌ Missing' })" -Level "INFO"
        Write-ColorLog "• Environment: $(if ($envSetup -eq 'configured') { '✅ Configured' } else { '❌ Missing' })" -Level "INFO"
        
        return @{
            RegistryCount = [int]$registryCount
            BundleSize = $bundleSize
            DockerCorpExists = ($dockerCorpExists -eq "exists")
        ...
        Write-ColorLog "Configuration verification failed: $($_.Exception.Message)" -Level "ERROR" -Color Red
        return $null
    }
}
...
function Install-DockerSSLConfiguration {
    ...
    try {
        ...
        # Step 5: Copy Docker environment script from repository file
        Write-ColorLog "Setting up Docker environment..." -Level "PROGRESS" -Color Yellow
        
        # Use file copy method for environment script as well
        $dockerEnvScript = Join-Path $PSScriptRoot "docker-corporate-env.sh"
        if (-not (Test-Path $dockerEnvScript)) {
            Write-ColorLog "ERROR: docker-corporate-env.sh not found in script directory" -Level "ERROR" -Color Red
            return $false
        }
...
Write-ColorLog "Performing final configuration verification..." -Level "TITLE" -Color Magenta
$configSummary = Test-ConfigurationSummary
...
# Use configuration summary results for more accurate assessment
if ($configSummary -and $configSummary.GoogleTest -and $configSummary.GitHubTest) {
    Write-ColorLog "PERFECT SUCCESS: docker-corp working for all test domains!" -Level "SUCCESS" -Color Green
} elseif ($configSummary -and ($configSummary.GoogleTest -or $configSummary.GitHubTest)) {
    Write-ColorLog "PARTIAL SUCCESS: docker-corp working for some domains!" -Level "SUCCESS" -Color Green
