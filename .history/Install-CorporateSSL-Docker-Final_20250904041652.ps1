...
function Install-DockerSSLConfiguration {
    ...
    try {
        ...
        # Step 4: Create Docker wrapper script using WSL directly (avoids PowerShell parsing issues)
        Write-ColorLog "Creating Docker wrapper script..." -Level "PROGRESS" -Color Yellow
        
        # Create script directly in WSL to avoid PowerShell parsing issues
        wsl bash -c @"
cat > /tmp/docker-corp << 'DOCKER_SCRIPT_EOF'
#!/bin/bash
...
sudo cp /tmp/docker-corp /usr/local/bin/docker-corp
sudo chmod +x /usr/local/bin/docker-corp
rm -f /tmp/docker-corp
"@ 2>$null
